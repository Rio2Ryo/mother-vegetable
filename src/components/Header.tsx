...
export default function Header() {
  ...
  const isLoggedIn = mounted && (!!currentUser || !!currentInstructor);
  const displayName = currentUser?.username || currentInstructor?.fullName || '';
                ...
                {isLoggedIn ? (
                  <>
                    <div className="py-3 px-4 border-2 border-[#25C760] rounded-lg">
                      ...
                      <p className="text-gray-400 text-xs mt-0.5">{currentUser?.email || currentInstructor?.email}</p>
                    </div>
                    {currentInstructor && (
                      <Link
