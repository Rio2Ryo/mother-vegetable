...
type ProductKey = 'sef' | 'aquaculture-small' | 'aquaculture-large' | 'agriculture';

const PRODUCT_LABELS: Record<ProductKey, { ja: string; en: string; zh: string }> = {
  sef: { ja: '1/100スケールSEF', en: '1/100 Scale SEF', zh: '1/100 Scale SEF' },
  'aquaculture-small': { ja: 'Mother Vegetable アクアカルチャーキット（小）', en: 'Mother Vegetable AquaCulture Kit - Small', zh: 'Mother Vegetable 水产养殖套件（小）' },
  'aquaculture-large': { ja: 'Mother Vegetable アクアカルチャーキット（大）', en: 'Mother Vegetable AquaCulture Kit - Large', zh: 'Mother Vegetable 水产养殖套件（大）' },
  agriculture: { ja: 'Mother Vegetable アグリカルチャーキット', en: 'Mother Vegetable AgriCulture Kit', zh: 'Mother Vegetable 农业种植套件' },
};

function getProductKey(value: string | null): ProductKey {
  if (value === 'aquaculture-small' || value === 'aquaculture-large' || value === 'agriculture') return value;
  return 'sef';
}
      ...
      <main className="min-h-screen bg-black text-white flex items-center justify-center px-6">
        <div className="max-w-lg text-center">
          <div className="w-16 h-16 rounded-full bg-[#25c760]/15 flex items-center justify-center mx-auto mb-6">
            <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#25c760" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12" /></svg>
          </div>
          <h1 className="text-2xl md:text-3xl font-bold mb-4">{t('Consultation Received', '申し込み相談を受け付けました', '咨询申请已提交')}</h1>
          <p className="text-gray-300 mb-8 leading-relaxed">
            {t('Thank you. We will confirm the conditions and contact you before any payment or contract procedure.', 'ありがとうございます。設置条件・内容を確認したうえで、決済または契約手続きの前に担当者からご連絡します。', '谢谢。我们会先确认条件，并在付款或合同流程前与您联系。')}
          </p>
      ...
      <section className="px-6 pb-24">
        <div className="max-w-2xl mx-auto border border-[#25c760]/30 rounded-2xl bg-[#0d1f12] p-6 md:p-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8 text-center">
            {['相談申込', '条件確認', '専用手続き'].map((step, index) => (
              <div key={step} className="rounded-xl bg-black/30 border border-[#25c760]/20 p-4">
                <p className="text-[#25c760] font-bold mb-1">STEP {index + 1}</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label htmlFor="product" className="block text-sm text-gray-300 mb-2">{t('Product', '対象商品', '对象商品')}</label>
              <input id="product" name="product" value={form.product} readOnly className="w-full px-4 py-3 rounded-lg bg-black/40 border border-[#25c760]/30 text-white" />
            </div>
            <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div>
                ...
                <select id="quantity" name="quantity" value={form.quantity} onChange={handleChange} className="w-full px-4 py-3 rounded-lg bg-black/40 border border-[#25c760]/30 text-white focus:outline-none focus:border-[#25c760]">
                  <option value="1">1</option><option value="2">2</option><option value="3">3</option><option value="相談">相談して決めたい</option>
                </select>
              </div>
            </div>
            <div>
            ...
            <button type="submit" className="w-full px-8 py-3 rounded-full bg-[#25c760] text-black font-semibold hover:bg-[#1da84e] transition-colors">{t('Submit Consultation', '申し込み相談を送信', '提交咨询')}</button>
            <p className="text-xs text-gray-500 text-center leading-relaxed">{t('Payment will not be made on this page. The dedicated payment or contract process will be provided after confirmation.', 'このページでは決済は行いません。確認後に専用の決済または契約手続きをご案内します。', '本页不会付款。确认后我们会提供专用付款或合同流程。')}</p>
          </form>
        </div>
      </section>
    </main>
